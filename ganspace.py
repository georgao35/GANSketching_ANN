...
def gen_principal_components(dump_name, device="cpu"):
    ...
    with np.load(dump_name) as data:
        lat_comp = jt.array(data["lat_comp"])
        lat_mean = jt.array(data["lat_mean"])
        lat_std = data["lat_stdev"]
    return lat_comp, lat_mean, lat_std


def apply_shift(
    g, mean_latent, latents, w_comp, w_std, s, layers, w_plus=False, trunc=0.5
):
    ...
    im: Var = g(
        [latents], input_is_latent=True, truncation=trunc, truncation_latent=mean_latent
    )[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--obj",
        type=str,
        choices=["cat", "horse", "church"],
        help="which StyleGAN2 class to use",
    )
    parser.add_argument(
        "--comp_id", type=int, required=True, help="which principle component to use"
    )
    parser.add_argument(
        "--scalar",
        type=float,
        required=True,
        help="strength applied to the latent shift, value can be negative",
    )
    parser.add_argument(
        "--layers",
        type=str,
        required=True,
        help="layers to apply GANSpace (e.g., 3,5 means layer 3 to 5",
    )
    parser.add_argument(
    ...
    lat_comp, lat_mean, lat_std = gen_principal_components(
        f"./weights/ganspace_{args.obj}.npz", device=device
    )
    w_comp = lat_comp[k]
    w_std = lat_std[k]
    ...
    if args.fixed_z is None:
        z = jt.array(np.random.randn(args.samples, 512))
    else:
    ...
    # generate outputs
    ims = apply_shift(
        netG, mean_latent, latents, w_comp, w_std, 0, layers, trunc=args.truncation
    )
    save_ims(f"./{args.save_dir}/before_", ims)
    ims = apply_shift(
        netG, mean_latent, latents, w_comp, w_std, s, layers, trunc=args.truncation
    )
