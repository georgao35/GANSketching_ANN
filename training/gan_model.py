...
class GANModel(Module):
    def __init__(self, opt):
        ...
        self.tf_real = networks.OutputTransform(
            opt, process=opt.transform_real, diffaug_policy=opt.diffaug_policy
        )
        self.tf_fake = networks.OutputTransform(
            opt, process=opt.transform_fake, diffaug_policy=opt.diffaug_policy
        )
    ...
    def forward(self, data, mode):
        ...
        if mode == "generator":
            ...
            d_loss, interm_imgs = self.compute_discriminator_loss(
                real_sketch, real_image
            )
            return d_loss, interm_imgs
        elif mode == "discriminator-regularize":
            assert (
                not self.opt.no_d_regularize
            ), "Discriminator shouldn't be regularized with --no_d_regularize applied"
            d_reg_loss = self.compute_discriminator_regularization(
                real_sketch, real_image
            )
            return d_reg_loss
        else:
    ...
    def create_optimizers(self, opt):
        ...
        if opt.optim_param_g == "style":
            ...
            raise ValueError(
                "--optim_param_g tag should be 'style' or 'w_shift', but get ",
                opt.optim_param_g,
            )
        ...
        d_reg_ratio = (
            opt.d_reg_every / (opt.d_reg_every + 1) if not opt.no_d_regularize else 1.0
        )
    ...
    def inference(self, noise, trunc_psi=1.0, mean_latent=None, with_tf=False):
        with jt.no_grad():
            ...
            if trunc_psi < 1 and mean_latent is None:
                mean_latent = self.get_mean_latent(
                    n_samples=self.opt.latent_avg_samples
                )

            img = self.netG(
                [noise], truncation=trunc_psi, truncation_latent=mean_latent
            )[0]
    ...
    def initialize_networks(self, opt):
        ...
        if opt.l_image > 0:
            assert (
                opt.dataroot_image is not None
            ), "dataset for image regularization needed"
    ...
    def compute_generator_loss(self):
        ...
        G_losses["G_sketch"] = self.criterionGAN(
        ...
        if self.opt.l_image > 0:
            ...
            G_losses["G_image"] = self.opt.l_image * self.criterionGAN(
                pred_fake2, True, for_discriminator=False
            )
    ...
    def compute_discriminator_loss(self, real_sketch, real_image):
        ...
        D_losses["D_fake_sketch"] = self.criterionGAN(
            pred_fake, False, for_discriminator=True
        )
        D_losses["D_real_sketch"] = self.criterionGAN(
        ...
        if self.opt.l_image > 0:
            ...
            D_losses["D_fake_image"] = self.opt.l_image * self.criterionGAN(
                pred_fake2, False, for_discriminator=True
            )
            D_losses["D_real_image"] = self.opt.l_image * self.criterionGAN(
                pred_real2, True, for_discriminator=True
            )
        ...
        if self.opt.reduce_visuals:
            ...
            interm_imgs = {
                "real_sketch": real_sketch.detach(),
                "fake_image": fake_image.detach(),
    ...
    def compute_discriminator_regularization(self, real_sketch, real_image):
        ...
        D_reg_losses["D_r1_loss_sketch"] = (
            self.opt.r1 / 2 * r1_loss * self.opt.d_reg_every
        )
        ...
        if self.opt.l_image > 0:
            ...
            D_reg_losses["D_r1_loss_image"] = (
                self.opt.l_image * self.opt.r1 / 2 * r1_loss2 * self.opt.d_reg_every
            )
    ...
    def generate_fake(self, batch_size=None, style_mix=True, return_latents=False):
        ...
        style_mix_prob = self.opt.mixing if style_mix else 0.0
...
def get_param_by_name(net: Module, tgt_param):
