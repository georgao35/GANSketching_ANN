...
class Evaluator:
    def __init__(self, opt, gan_model):
        ...
        if opt.resume_iter is not None:
            load_path = os.path.join(
                self.opt.checkpoints_dir, self.opt.name, "best_FID.npy"
            )
    ...
    def run_metrics(self, iters):
        ...
        with mp.Pool(1) as p:
            metrics = p.apply(
                metrics_process, (cache_folder, self.fid_stat, self.real_vgg,)
            )
...
def metrics_process(cache_folder, fid_stats, vgg_feats):
    ...
    fake_feats = pr.get_features(
        f"{cache_folder}/image/", feature_net, 2500, 10, num_gpus=1
    )
