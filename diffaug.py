...
def rand_saturation(x):
    ...
    x = (x - x_mean) * (jittor.rand(x.size(0), 1, 1, 1,
                                    dtype=x.dtype) * 2) + x_mean
    return x
...
def rand_translation(x, ratio=0.125):
    ...
    translation_x = jittor.randint(-shift_x, shift_x + 1,
                                   size=[x.size(0), 1, 1])
    translation_y = jittor.randint(-shift_y, shift_y + 1,
                                   size=[x.size(0), 1, 1])
    ...
    x = x_pad.permute(0, 2, 3, 1)[grid_batch, grid_x, grid_y].permute(
        0, 3, 1, 2)  # TODO contiguous substitute correct?
    return x

def rand_cutout(x, ratio=0.5):
    cutout_size = int(x.size(2) * ratio + 0.5), int(x.size(3) * ratio + 0.5)
    offset_x = jittor.randint(0, x.size(2) + (1 - cutout_size[0] % 2), size=[x.size(0), 1, 1])
    offset_y = jittor.randint(0, x.size(3) + (1 - cutout_size[1] % 2), size=[x.size(0), 1, 1])
    grid_batch, grid_x, grid_y = jittor.meshgrid(
    ...
    grid_x = jittor.clamp(grid_x + offset_x - cutout_size[0] // 2, min=0, max=x.size(2) - 1)
    grid_y = jittor.clamp(grid_y + offset_y - cutout_size[1] // 2, min=0, max=x.size(3) - 1)
