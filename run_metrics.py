...
def make_eval_images(g, save_folder, eval_samples, batch_size, device, to_cpu=True):
    ...
    for i in range(iterations):
        ...
        for ind in range(sample.size(0)):
            save_image_pytorch(
                sample[ind], f"{save_folder}/image/{str(img_count).zfill(6)}.png"
            )
            img_count += 1
...
def get_metrics(opt, name, target):
    ...
    fid_value = fid.compute_fid(
        real_folder + "image", fake_folder + "image", num_workers=0
    )

    ppl_wend = compute_ppl(
        g,
        num_samples=50000,
        epsilon=1e-4,
        space="w",
...
def get_stats(opt, g, folder):
    ...
    if os.path.exists(f"{folder}/image/"):
        if (
            len([s for s in os.listdir(f"{folder}/image/") if s.endswith(".png")])
            == opt.eval_samples
        ):
            file_cached = True
...
def run_vgg(folder, eval_samples, batch_size):
    ...
    features = pr.get_features(
        f"{folder}/image/", feature_net, eval_samples, batch_size, num_gpus=1
    )
